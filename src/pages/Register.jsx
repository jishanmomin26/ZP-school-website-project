...
const Register = () => {
  ...
  const [form, setForm] = useState({
    ...
    parentId: '', studentName: '', studentClass: '',
  });
      ...
      <section className="min-h-screen flex items-center justify-center bg-gradient-to-br from-dark-50 via-primary-50/30 to-dark-50 py-20 px-4">
        ...
        >
          <div className="text-center mb-8">
            ...
            <h1 className="font-poppins text-2xl font-bold text-dark-800">{t('register_title')}</h1>
            <p className="text-dark-500 text-sm mt-1">{t('register_subtitle')}</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-dark-100 overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <InputField label={t('register_name')} name="name" placeholder={t('register_name')} error={errors.name} />

              {role === 'teacher' ? (
                <InputField label={t('register_email')} name="email" type="email" placeholder="teacher@example.com" error={errors.email} />
              ) : (
                <>
                  <InputField label={t('register_parent_id')} name="parentId" placeholder="PARENT002" error={errors.parentId} />
                  <InputField label={t('register_student_name')} name="studentName" placeholder={t('register_student_name')} error={errors.studentName} />
                  <div>
                    <label className="block text-sm font-medium text-dark-700 mb-1.5">{t('register_class')} *</label>
                    <select
              ...
              <InputField label={t('register_phone')} name="phone" type="tel" placeholder="+91 9876543210" error={errors.phone} />

              <div>
                <label className="block text-sm font-medium text-dark-700 mb-1.5">{t('register_password')} *</label>
              ...
              <InputField label={t('register_confirm')} name="confirm" type="password" placeholder="••••••••" error={errors.confirm} />

              <button type="submit" className="btn-primary w-full">{t('register_button')}</button>
