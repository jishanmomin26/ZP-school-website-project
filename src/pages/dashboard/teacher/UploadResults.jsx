...
const UploadResults = () => {
  ...
  useEffect(() => {
    ...
    const existingResults = saved[selectedClass]?.[selectedExam] || defaultResults[selectedClass]?.[selectedExam] || [];

    const initial = {};
    filteredStudents.forEach(s => {
      const existing = existingResults.find(r => r.studentId === s.id);
      initial[s.id] = existing || { marathi: '', english: '', maths: '', evs: '' };
    });
    setResults(initial);
  ...
  const handleSave = () => {
    const saved = JSON.parse(localStorage.getItem('zpkudave_results') || '{}');
    if (!saved[selectedClass]) saved[selectedClass] = {};
    saved[selectedClass][selectedExam] = Object.entries(results).map(([id, marks]) => ({
      studentId: Number(id), ...marks,
    }));
    localStorage.setItem('zpkudave_results', JSON.stringify(saved));
    toast.success(t('teacher_results_saved'));
  };

  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      <h1 className="font-poppins text-2xl font-bold text-dark-800 mb-6">{t('dash_upload_results')}</h1>

      {/* Selectors */}
      <div className="flex flex-wrap gap-4 mb-6">
        <div>
          <label className="block text-xs font-medium text-dark-500 mb-1">{t('teacher_select_class')}</label>
          <div className="flex gap-2">
            {['1', '2', '3', '4'].map(cls => (
              <button key={cls} onClick={() => setSelectedClass(cls)}
                className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${
                  selectedClass === cls ? 'bg-primary-600 text-white shadow-md' : 'bg-white text-dark-600 border border-dark-200 hover:border-primary-300'
                }`}>{t('teacher_class')} {cls}</button>
            ))}
          </div>
        </div>
        <div>
          <label className="block text-xs font-medium text-dark-500 mb-1">{t('teacher_select_exam')}</label>
          <select value={selectedExam} onChange={e => setSelectedExam(e.target.value)} className="input-field w-auto py-2">
            {exams.map(e => <option key={e} value={e}>{e}</option>)}
          </select>
        </div>
      </div>

      <p className="text-dark-400 text-sm mb-4">Max marks: {maxMarks} per subject</p>
              ...
              {filteredStudents.map((student, i) => {
                ...
                return (
                  <motion.tr key={student.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-dark-600">{student.roll}</td>
                    <td className="px-4 py-3 text-sm font-medium text-dark-800">{student.name}</td>
                    {subjects.map(sub => (
                    ...
                    <td className="px-4 py-3 text-center font-semibold text-dark-800">{total}</td>
